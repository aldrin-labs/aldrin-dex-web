...
export const PRICE_HISTORY_QUERY = gql`
  query priceHistoryQuery($coins: [String!], $isBTC: Boolean!, $unixTimestampFrom: Int!, $unixTimestampTo: Int!) {
    getPriceHistory(
        coins: $coins,
        isBTC: $isBTC,
                ...
                >
                    ...
                    >
                        ...
                        <Crosshair values={crosshairValues}>
                            ...
                            >
                                <p>
                                    {crosshairValues.map((v) => new Date(v.x * 1000).toDateString()).join(' ')}:{' '}
                                    {crosshairValues.map((v) => `$${Number(v.y).toFixed(2)}`).join(' ')}
                                </p>
                            </div>
                        </Crosshair>
...
export default class GQLChart extends React.Component {
    ...
    static getDerivedStateFromProps(newProps, state) {
        if (newProps.coins !== state.coins) {
            ...
            newState.coins = newProps.coins.map(x => x.symbol).filter(x => x !== 'FUN');
            // tslint:disable-next-line:no-object-mutation
            newState.assets = newProps.coins;
            // tslint:disable-next-line:no-object-mutation
            newState.sum = newProps.coins.map(x => x.quantity).reduce((prev, next) => prev + next, 0);
    ...
    getTimestampRange(days) {
        ...
        return { left: Math.floor((today - days * 24 * 60 * 60)), right: Math.floor(today) }
    }
